...
namespace NuGetGallery
{
    public class UploadFileService : IUploadFileService
    {
        ...
        {
            if (userKey < 1) throw new ArgumentException("A user key is required.", "userKey");

            var uploadFileName = BuildFileName(userKey);

            return fileStorageService.GetFile(Constants.UploadsFolderName, uploadFileName, useCache: false);
        }
