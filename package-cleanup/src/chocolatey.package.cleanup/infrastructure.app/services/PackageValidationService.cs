...
namespace chocolatey.package.cleanup.infrastructure.app.services
{
    ...
    using locators;
    using rules;
    using IPackageRule = rules.IPackageRule;

    public class PackageValidationService : IPackageValidationService
    {
        private readonly ITypeLocator _typeLocator;
        private readonly IList<IPackageRule> _rules = new List<IPackageRule>();
        ...
        public IEnumerable<PackageValidationResult> validate_package(IPackage package)
        {
            ...
            if (package == null) results.Add(new PackageValidationResult(false, "Something went very wrong. There is no package to validate. Please contact the site admins for next steps.", ValidationLevelType.Requirement));
            else
            {
