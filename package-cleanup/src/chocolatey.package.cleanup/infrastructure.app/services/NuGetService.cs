...
namespace chocolatey.package.cleanup.infrastructure.app.services
{
    ...
    public class NuGetService : INuGetService
    {
        ...
        public IPackage download_package(string packageId, string packageVersion, string downloadLocation, IConfigurationSettings configuration)
        {
            ...
            {
                Bootstrap.handle_exception(new ApplicationException("Encountered error downloading package {0} v{1}:{2}{3}".format_with(packageId, packageVersion, Environment.NewLine, ex.to_string()), ex));
                return null;
            }
