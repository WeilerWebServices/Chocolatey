...
namespace chocolatey.package.cleanup.infrastructure.synchronization
{
    using System;
    ...
    public class TransactionLock
    {
        ...
        public static void enter(string name, int? secondsToTimeout, Action action)
        {
            ...
            try
            {
                ...
                else throw new ApplicationException("Was not able to Acquire a lock on '{0}' within '{1}' seconds.".format_with(name, secondsToTimeout));
            }
            catch (Exception)
            {
                throw;
