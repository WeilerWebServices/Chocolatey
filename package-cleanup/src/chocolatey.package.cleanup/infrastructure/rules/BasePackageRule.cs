...
namespace chocolatey.package.cleanup.infrastructure.rules
{
    ...
    public abstract class BasePackageRule : IPackageRule
    {
        ...
        {
            var name = GetType().Name.to_lower();
            if (name.EndsWith("requirement")) ValidationLevel = ValidationLevelType.Requirement;
            else if (name.EndsWith("suggestion")) ValidationLevel = ValidationLevelType.Suggestion;
            else if (name.EndsWith("note")) ValidationLevel = ValidationLevelType.Note;
            else ValidationLevel = ValidationLevelType.Guideline;
        }
